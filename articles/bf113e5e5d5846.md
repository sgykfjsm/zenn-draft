...
`NO CACHE`をつけることで連番がキャッシュされず、常に１つずつ採番されるようになります。キャッシュする場合、デフォルトでは各TiDBノードで1,000ずつキャッシュします。これはAUTO_INCREMENTと同じような動作になるのですが、つまり、TiDBノードでキャッシュしなくなることで、TiDBノードが部分的に利用できなくなっても、あるいはスケールアウトにより追加されたとしても、どこで採番されても必ず連続したIDとなります。
